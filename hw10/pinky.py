...
class Pinky(GameCharacter):
    ...
    def update(self):
        ...
        on_horz = (abs(self.y - self.maze.TOP_HORIZ) < self.WALL_TOLERANCE) or (
        ...
        # that Pinky wins
        if (
            abs(up_down_part) < self.WIN_PROXIMITY
        ...
        # START CODE CHANGES
        # Pinky's AI: Decide direction at intersections
        if on_vert and on_horz:
            if abs(left_right_part) > abs(up_down_part):
                # Move horizontally
                if left_right_part > 0:
                    self.x_add = self.velocity  # Move right
                    self.y_add = 0
                else:
                    self.x_add = -self.velocity  # Move left
                    self.y_add = 0
            else:
            # Move vertically
                if up_down_part > 0:
                  self.y_add = self.velocity  # Move down
                  self.x_add = 0
                else:
                  self.y_add = -self.velocity  # Move up
                  self.x_add = 0
