...
class Pacman(GameCharacter):
    ...
    def update(self):
        ...
        drawing to screen"""
        self.on_left = abs(self.x - self.maze.LEFT_VERT) < self.WALL_TOLERANCE
        self.on_right = (
            abs(self.x - self.maze.RIGHT_VERT) < self.WALL_TOLERANCE
        )

        self.on_top = abs(self.y - self.maze.TOP_HORIZ) < self.WALL_TOLERANCE
        self.on_bottom = (
        ...
        elif self.mouth_angle <= self.min_mouth_angle:
            self.open_close = self.mouth_speed
        ...
        # BEGIN CODE CHANGES
        if self.on_top:
            self.maze.eat_dots("TOP", self.x)
        if self.on_bottom:
            self.maze.eat_dots("BOTTOM", self.x)
        if self.on_left:
            self.maze.eat_dots("LEFT", self.y)
        if self.on_right:
            self.maze.eat_dots("RIGHT", self.y)
